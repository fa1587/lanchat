...
The image viewer SHALL provide a close button and display the file name. The AppBar and controls SHALL appear on a semi-transparent overlay that does not obscure the image.

#### Scenario: Close viewer

- **WHEN** user taps the close (✕) button or presses the system back button
