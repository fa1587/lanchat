...
The transfer state layer manages the lifecycle of file transfer records. It splits transfers into two reactive streams — active (pending/transferring) and history (completed/failed/cancelled) — and provides methods to dismiss individual history entries or clear the entire history.
