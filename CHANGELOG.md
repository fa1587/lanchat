...
## 1.6.1 (2026-06-26)
- [All] 🐛 修复离线设备无法点击进入聊天查看历史记录
- [All] 🐛 离线聊天时禁用输入栏，避免无效发送

## 1.0.14 (2026-06-17)
- [All] 🔴 主界面未读消息红点气泡（设备列表右侧显示未读计数）
- [Win] 🐛 修复 Windows 端 SQLite 不可用（加 sqflite_common_ffi 依赖）
- [All] 🐛 修复吞第一条消息气泡的 Bug（异步写入时序）
- [All] 🐛 修复打开聊天不显示历史消息（异步 DB 加载）
